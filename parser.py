...
def find_poster_image_src(soup):
    ...
    return strip_media_amazon_url(img_poster["src"], delimeter='@@' if '@@' in img_poster['src'] else '@')
...
def find_movie_photos_src(soup, limit=4):
    ...
    return [strip_media_amazon_url(img['src'], delimeter='@@' if '@@' in img['src'] else '@') for img in img_photos]
